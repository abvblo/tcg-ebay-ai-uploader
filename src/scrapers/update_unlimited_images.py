...
from src.database.models import PokemonCard, CardVariation
...
class UnlimitedImageUpdater:
    ...
    def __init__(self, db_url: str = None):
        ...
        if not db_url:
            ...
            load_dotenv()
            db_url = os.getenv(
                "DATABASE_URL", "postgresql://mateo:pokemon@localhost:5432/pokemon_cards"
            )
        ...
        self.web_image_dir = Path(__file__).parent.parent.parent / "assets/images/pokemon/english/base1"
        self.web_image_dir.mkdir(parents=True, exist_ok=True)

    def get_base_set_cards(self):
        """Get all Base Set cards from database"""
        return self.session.query(PokemonCard).filter(PokemonCard.set_id == "base1").all()  # Base Set ID

    def update_card_with_unlimited_image(self, card: PokemonCard, image_path: str) -> bool:
        ...
        try:
            ...
            unlimited_variation = (
                ...
                .filter(CardVariation.card_id == card.id, CardVariation.variation_type == "unlimited")
                .first()
            )

            if unlimited_variation:
                # Note: CardVariation model doesn't have image_url field
                # Images are stored in the main PokemonCard.images JSONB field
                self.session.commit()
    ...
    def update_base_set_unlimited(self):
        ...
        scraper = BulbapediaScraper(
            output_dir="scraped_images/base_set", delay=2.0  # Respectful delay
        )
