...
from sqlalchemy import create_engine, Column, String, Integer, Float, Boolean, DateTime, Date, Text, ForeignKey, Index, UniqueConstraint, Table
...
class PokemonSet(Base):
    ...
    __table_args__ = (
        Index('idx_set_search', 'search_vector', postgresql_using='gin'),
    )
...
class XimilarCorrection(Base):
    ...
    __table_args__ = (
        Index('idx_correction_ximilar', 'ximilar_result', postgresql_using='gin'),
    )
