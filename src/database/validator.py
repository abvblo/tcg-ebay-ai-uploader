...
from .models import (
    PokemonCard, PokemonSet, CardVariation, 
    XimilarCorrection, ValidationRule
)
...
class DatabaseValidator:
    ...
    def validate_card(self, ximilar_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fuzzy_matches:
            ...
            if match_score > 90:  # High confidence fuzzy match
                ...
                result['validation_notes'].append(f"Low confidence with fuzzy match (score: {match_score})")
                result['suggested_correction'] = {
                    'name': best_match.name,
                    'set': best_match.set.name,
                    'number': best_match.number
    ...
    def _find_exact_match(self, name: str, set_name: str, number: str) -> Optional[PokemonCard]:
        ...
        card = self.session.query(PokemonCard).join(PokemonSet).filter(
            and_(
        ...
        # Try with partial set name match
        card = self.session.query(PokemonCard).join(PokemonSet).filter(
            and_(
                func.lower(PokemonCard.name) == name.lower(),
                PokemonSet.name.ilike(f"%{set_name}%"),
                PokemonCard.number == number
            )
        ).first()
    ...
    def _find_fuzzy_matches(self, name: str, set_name: str, number: str) -> List[Tuple[PokemonCard, float]]:
        """Find fuzzy matches with scores"""
        matches = []
        
        # Get potential matches by name
        potential_cards = self.session.query(PokemonCard).filter(
            PokemonCard.name.ilike(f"%{name.split()[0]}%")
        ).limit(50).all()
        ...
        for card in potential_cards:
            ...
            # Number matching
            number_score = 100 if number == card.number else (
                80 if number.split('/')[0] == card.number.split('/')[0] else 0
            )
    ...
    def _identify_variation(self, card: PokemonCard, ximilar_result: Dict[str, Any]) -> Optional[CardVariation]:
        ...
        is_first_ed = any(term in str(ximilar_result).lower() for term in ['1st edition', 'first edition'])
        is_stamped = any(term in str(ximilar_result).lower() for term in ['stamped', 'staff', 'league'])
        
        # Find matching variation
        variations = card.variations
        
        for var in variations:
            if (var.is_reverse_holo == is_reverse and 
                var.is_first_edition == is_first_ed and
                var.is_stamped == is_stamped):
                return var
    ...
    def _run_validation_rules(self, ximilar_result: Dict[str, Any]) -> List[str]:
        ...
        rules = self.session.query(ValidationRule).filter_by(is_active=True).order_by(
            ValidationRule.priority.desc()
        ).all()
    ...
    def _check_set_size_rule(self, config: Dict, ximilar_result: Dict) -> bool:
        ...
        try:
            ...
            # Look up actual set size
            set_name = ximilar_result.get('set_name', '')
            actual_set = self.session.query(PokemonSet).filter(
                PokemonSet.name.ilike(f"%{set_name}%")
            ).first()
            
            if actual_set and actual_set.printed_total:
                # Allow for secret rares (up to 50 beyond printed total)
                if set_total != actual_set.printed_total and abs(set_total - actual_set.printed_total) > 50:
                    return True
        except:
            pass
    ...
    def save_correction(self, ximilar_result: Dict[str, Any], 
