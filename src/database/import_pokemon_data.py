...
class PokemonDataImporter:
    ...
    async def _import_card(self, card_data: Dict[str, Any]):
        ...
        card = PokemonCard(
            ...
            tcgplayer_id=str(card_data['tcgplayer'].get('productId')) if card_data.get('tcgplayer') and card_data['tcgplayer'].get('productId') else None,
    ...
    async def _create_variations(self, card: PokemonCard, tcgplayer_data: Dict[str, Any]):
        ...
        for price_type, price_data in tcgplayer_data['prices'].items():
            ...
            existing_var = self.session.query(CardVariation).filter_by(
                card_id=card.id,
