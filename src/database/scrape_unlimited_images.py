...
from models import PokemonCard, PokemonSet, DatabaseConfig, get_session
import re
...
class UnlimitedImageScraper:
    ...
    async def search_tcgplayer_for_card(self, session: aiohttp.ClientSession, card_name: str, card_number: str):
    ...
    async def download_from_tcgplayer(self, session: aiohttp.ClientSession, card: PokemonCard, output_dir: Path):
    ...
    async def scrape_unlimited_images(self):
        ...
        try:
            ...
            unlimited_dir = project_root / 'src' / 'web' / 'static' / 'images' / 'pokemon' / 'base-unlimited'
            ...
            cards = session.query(PokemonCard).filter_by(set_id=base_set.id).order_by(
                PokemonCard.number.cast(Integer)
            ).all()
