...
class ImageOptimizer:
    ...
    async def optimize_image_list_async(
        self, image_paths: List[Path], create_backup: bool = True
    ) -> List[Path]:
        """
        Async version that uses concurrent processing for better performance
        """
        # Create async optimizer instance
        async_optimizer = AsyncImageOptimizer(
        ...
        result = await async_optimizer.optimize_image_list(image_paths, create_backup)

        # Update our stats from async optimizer
        stats = async_optimizer.get_optimization_stats()
        self.optimized_count += stats["optimized_count"]
        self.total_size_saved += int(stats["total_size_saved_mb"] * 1024 * 1024)
