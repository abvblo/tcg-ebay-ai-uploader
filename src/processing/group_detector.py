...
class ImageGroupDetector:
    ...
    def _group_remaining_files(
        ...
        for img_path in remaining_files:
            ...
            existing_group = next(
                (g for g in groups if g.key == group_key and not g.is_sequential_pair), None
            )
            if existing_group:
                existing_group.paths.append(img_path)
            else:
                groups.append(ImageGroup(key=group_key, paths=[img_path], is_sequential_pair=False))
    ...
    # Async method for enhanced performance
    async def find_groups_async(self) -> List[ImageGroup]:
        """
        Async version that uses concurrent processing for better performance
        """
        from .async_group_detector import AsyncImageGroupDetector

        # Create async detector instance
        async_detector = AsyncImageGroupDetector(self.config)

        # Run async group detection
        return await async_detector.find_groups()
