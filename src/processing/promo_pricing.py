...
class PromoPricingCalculator:
    ...
    HIGH_VALUE_PROMO_KEYWORDS = [
        'staff', 'championship', 'worlds', 'regional', 'national',
        'winner', 'participant', 'judge', 'professor', 'league leader',
        'prerelease', 'launch'
    ]
    
    def get_fallback_price(self, card_name: str, set_name: str, 
                          rarity: str = None, card_number: str = None,
                          unique_characteristics: list = None) -> Tuple[float, str]:
        """
        ...
        if 'promo' in set_name_lower or self._has_promo_number(card_number):
            if any(char in str(unique_characteristics).lower() 
                   for char in ['holo', 'foil', 'reverse']):
                price = 3.49
