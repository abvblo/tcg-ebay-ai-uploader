...
class XimilarDebugger:
        ...
        if distances:
            ...
            if analysis["confidence_score"] < 0.5:
                analysis["issues"].append(
                    f"Very low confidence: {analysis['confidence_score']:.2%}"
                )
    ...
    def validate_and_suggest(
        self, identification: Dict, alternatives: List[Dict]
    ...
    def generate_report(self) -> str:
        ...
        report_path = (
            self.output_dir / f"debug_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
