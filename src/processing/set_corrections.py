...
class SetCorrector:
        ...
        try:
            ...
            if is_secret_rare:
                ...
                if num > total + 50:
                    ...
                    logger.warning(
                        f"   ⚠️ Card number {num} is unusually high for set total {total}"
                    )
                    return False
                else:
            ...
            for known_set, correct_total in self.KNOWN_SET_SIZES.items():
                if known_set.lower() in set_name.lower():
                    if total != correct_total:
                        ...
                        logger.warning(
                            f"   ⚠️ Set total {total} doesn't match known size {correct_total} for {known_set}"
                        )
    ...
    def suggest_corrections(
        self, card_name: str, set_name: str, confidence: float
    ) -> Optional[str]:
        """
        ...
        if confidence < 0.7:
            ...
            if any(
                term in card_name.lower() for term in ["promo", "staff", "championship", "winner"]
            ):
    ...
    def detect_and_correct_promo_patterns(
        self, card_name: str, set_name: str, card_number: str
            ...
            if prefix in promo_set_map:
                ...
                if set_name != correct_set:
                    ...
                    return (
                        correct_set,
                        correct_number,
                        f"Promo number {correct_number} indicates {correct_set}",
                    )

        # Check for Staff/Championship patterns
        if any(
            term in card_name.lower()
            for term in ["staff", "championship", "city", "state", "regional", "worlds"]
        ):
            if "black star promo" not in set_name.lower() and "promo" not in set_name.lower():
                ...
                if any(era in set_name.lower() for era in ["sun & moon", "sm"]):
                    return (
                        ...
                        card_number,
                        "Tournament promos are in Black Star sets",
                    )
