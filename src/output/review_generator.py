...
class ReviewGenerator:
    ...
    def generate_review_html(self, cards: List[CardData]) -> str:
        ...
        cards_to_review = [
            ...
            if card.review_flag != 'OK' or 
            any(term in card.name.lower() for term in [' v', ' vmax', ' gx', ' ex', 'break'])
        ]
...
"""
        ...
        for card in cards_to_review:
            ...
            confidence_class = (
                'high-confidence' if confidence_pct >= 95 else
                'medium-confidence' if confidence_pct >= 90 else
                'low-confidence'
            )
