...
class EbayFormatter:
    ...
    def format_card(self, card: CardData, title: str, index: int) -> Dict[str, Any]:
        """Format card data for eBay upload - DATABASE-ENHANCED VERSION"""
        # Format image URLs with pipe delimiter
        image_url = ""
        if card.image_urls:
            urls_with_cache_bust = [
                f"{url}?cache-bust={i}" for i, url in enumerate(card.image_urls)
            ]
            image_url = "|".join(urls_with_cache_bust)

        # Use database fields when available
        db_card = getattr(card, "database_card", None)
        db_variation = getattr(card, "database_variation", None)

        # Extract year from release date (database first)
        year_manufactured = self._extract_year_from_database(card, db_card)
        ...
        return {
            ...
            # Item specifics - DATABASE ENHANCED
            "*C:Game": f"{card.game} TCG",
            "C:Card Name": db_card.name if db_card else card.name,
            "C:Character": (
                (db_card.name if db_card else card.name).split()[0]
                if (db_card or card.name)
                else ""
            ),
            "C:Set": db_card.set.name if db_card and db_card.set else card.set_name,
            "C:Rarity": db_card.rarity if db_card else card.rarity,
            "C:Card Number": db_card.number if db_card else card.number,
            "C:Graded": "No",
            "C:Manufacturer": self._get_manufacturer(card.game),
            "C:Autographed": "No",
            "C:Language": card.language,
            "C:Finish": self._get_finish_value_from_database(card, db_card, db_variation),
            "C:Features": self._get_features_from_database(card, db_card, db_variation),
            "C:Artist": db_card.artist if db_card and db_card.artist else "",
            # NEW FIELDS
            ...
            # Pokemon/MTG specific - DATABASE ENHANCED
            "C:HP": str(db_card.hp) if db_card and db_card.hp else (card.hp or ""),
            "C:Card Type": self._get_card_types_from_database(card, db_card),
            "C:Attribute/MTG:Color": self._get_card_colors_from_database(card, db_card),
            "C:Supertype": db_card.supertype if db_card else "",
    ...
    def _get_shipping_policy(self, price: float) -> str:
        ...
        if price < 20.00:
            return self.business_policies.get(
                "shipping_policy_under_20", "Standard Envelope 1oz (Free)"
            )
        else:
...
{details_html}
  ...
  <p><strong>– The AboveBelow Fam</strong> 🃏💚</p>
    ...
    def _get_finish_value_from_database(self, card: CardData, db_card, db_variation) -> str:
        """Get finish value from database variation - only for holo/foil finishes"""
        if db_variation and db_variation.finish:
            finish = db_variation.finish.lower()
            # Map database finishes to eBay values - only special finishes
            if finish == "holofoil":
                return "Holo"
        ...
        # Check variation flags
        if db_variation and db_variation.is_reverse_holo:
            return "Reverse Holo"

        # Fallback to original method
        return self._get_finish_value(card)

    def _get_features_from_database(self, card: CardData, db_card, db_variation) -> str:
        """Get features from database variation and characteristics"""
        features = []

        # Add database variation features
        if db_variation:
            ...
            if db_variation.is_stamped:
                stamp_text = db_variation.stamp_type or "Stamped"
                features.append(stamp_text)
            if db_variation.is_promo:
                features.append("Promo")

        # Add Ximilar characteristics as fallback
        if card.unique_characteristics:
            features.extend(card.unique_characteristics)

        # Remove duplicates and return
        unique_features = list(dict.fromkeys(features))  # Preserve order
        return ", ".join(unique_features)

    def _get_card_types_from_database(self, card: CardData, db_card) -> str:
        """Get card types from database"""
        if db_card and db_card.subtypes:
            # Convert relationship to list of names
            subtype_names = [subtype.name for subtype in db_card.subtypes]
            return ", ".join(subtype_names)

        # Fallback to original data
        return ", ".join(card.subtypes) if card.subtypes else ""

    def _get_card_colors_from_database(self, card: CardData, db_card) -> str:
        """Get card colors/types from database"""
        if db_card and db_card.types:
            # Convert relationship to list of names
            type_names = [card_type.name for card_type in db_card.types]
            return ", ".join(type_names)

        # Fallback to original data
        return ", ".join(card.types) if card.types else ""

    def _extract_year_from_database(self, card: CardData, db_card) -> str:
        """Extract year from database release date"""
        if db_card and db_card.set and db_card.set.release_date:
            return str(db_card.set.release_date.year)

        # Fallback to original method
        return self._extract_year(card)
