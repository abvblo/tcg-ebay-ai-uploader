...
class ExcelGenerator:
    ...
    def __init__(self, config: Config, metrics: Optional[MetricsTracker] = None):
        ...
        if config.openai_api_key:
            self.title_optimizer = OpenAITitleOptimizer(
                ...
                rate_limit=self.config.processing.rate_limit_openai
            )
        else:
    ...
    async def create_excel(self, cards: List[CardData]) -> str:
                ...
                for j, (card, title) in enumerate(zip(chunk, chunk_titles)):
                    if card.is_variation_parent and card.variations:
                        ...
                        variation_rows = self._create_variation_listing(
                            card, 
                            title, 
                            len(all_ebay_rows) + len(chunk_rows)
                        )
        ...
        if self.metrics:
            self.metrics.record_excel_generation(
                num_listings=len(cards),
                num_rows=len(all_ebay_rows),
    ...
    async def _optimize_titles_batch(
        ...
        for i in range(0, len(card_dicts), batch_size):
            ...
            try:
                ...
                # Log progress
                logger.debug(f"Optimized titles for batch {i//batch_size + 1}/{(len(card_dicts)-1)//batch_size + 1}")
                ...
                fallback_titles = [self._generate_fallback_title(card) for card in cards[i:i + batch_size]]
    ...
    def _create_variation_listing(
        ...
        parent_row.update({
            ...
            'CustomLabel': parent_card.variations[0]['custom_label'] if parent_card.variations else ''
        })
    ...
    def _create_variation_row(
        ...
        if variation.get('image_urls'):
            cache_busted_urls = [
                f"{url}?cb={i}" 
                for i, url in enumerate(variation['image_urls'])
                if url
            ]
        ...
        for field in [
            '*Category', '*ConditionID', '*Description', '*Format', 
            '*Duration', '*Location', 'PostalCode', '*DispatchTimeMax',
            'PaymentProfileName', 'ShippingProfileName', 'ReturnProfileName',
            'ConfidenceScore', 'ReviewFlag', 'PriceSource', 'TCGPlayerLink',
            'ProcessingNotes'
        ]:
            row[field] = ''
    ...
    def _write_to_excel(self, rows: List[Dict[str, Any]], output_path: Path) -> None:
        ...
        with pd.ExcelWriter(
            output_path,
            engine='openpyxl',
            engine_kwargs={'mode': 'w'},
            options={
                'strings_to_urls': False,
                'strings_to_formulas': False,
                'strings_to_numbers': True,
                'strings_to_datetime': False,
                'encoding': 'utf-8',
                'freeze_panes': (1, 0)
            }
        ) as writer:
